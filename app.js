...
function lineContactUrl(product){
 const lineId=String(product?.lineId||"@sample-machinery").trim();
 const message=`「${product?.title||"商品"}」についてお問い合わせです。`;
...
async function loadProductDetail(){
 ...
 try{
  ...
  el.innerHTML=`<section class="detail"><div class="container"><a class="muted" href="/products.html">← すべての機械</a><div class="detail-grid" style="margin-top:22px"><div><img id="mainImg" class="gallery-main" src="${main}" alt="${escapeHtml(p.title)}"><div class="thumbs">${thumbs}</div></div><div><div class="eyebrow">${escapeHtml(p.category||"Machine")}</div><h1>${escapeHtml(p.title)}</h1><p class="lead">${escapeHtml(p.description)}</p><div class="price">${escapeHtml(money(p))}</div><div class="contact-box"><h3>この機械にご興味がありますか？</h3><p class="muted">LINEから直接お問い合わせください。</p><a class="btn line" href="${lineContactUrl(p)}" target="_blank" rel="noopener">LINEで問い合わせる</a><div style="margin-top:15px"><img class="qr" src="/assets/line-qr.png" onerror="this.src='/assets/line-qr.svg'" alt="LINE QR code"></div></div></div></div>
