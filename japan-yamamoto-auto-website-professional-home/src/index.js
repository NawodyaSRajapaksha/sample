...
async function hashPassword(password) {
  ...
  return `pbkdf2-sha256$${PBKDF2_ITERATIONS}$${base64url(
    salt
...
async function verifyPassword(password, stored) {
  try {
    const [scheme, iterText, saltText, hashText] =
      String(stored).split("$");
...
function validateProduct(body) {
  ...
  if (!title) {
    return {
      error: "機械タイトルを入力してください。"
    };
  }

  const category = String(
    body.category || "ミニショベル"
  ).trim();
...
async function handleApi(request, env, url) {
  ...
  // INITIAL SETUP
  // No SETUP_TOKEN is required.
  // Setup is possible only when there are zero admins.
  if (
    ...
    await env.DB
      .prepare(
        `INSERT INTO products
         (id,title,category,description,video_url,
          price_mode,price,status,images_json,
