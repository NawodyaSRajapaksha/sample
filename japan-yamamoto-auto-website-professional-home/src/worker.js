...
async function verifyAdmin(request, env) {
  ...
  const row = await env.DB.prepare(
    "SELECT s.email, s.expires_at FROM yamamoto_sessions s WHERE s.token = ?"
  ).bind(token).first();
  if (!row || Number(row.expires_at) <= Math.floor(Date.now() / 1000)) return null;
  const admin = await env.DB.prepare("SELECT email FROM yamamoto_admins WHERE email = ?").bind(row.email).first();
  return admin || null;
}
...
async function handleLogin(request, env) {
  ...
  const admin = await env.DB.prepare("SELECT email, password_hash, salt FROM yamamoto_admins WHERE lower(email)=? LIMIT 1")
    .bind(email).first();
  ...
  await env.DB.prepare(
    "INSERT INTO yamamoto_sessions(token,email,expires_at,created_at) VALUES (?,?,?,?)"
...
async function ensureBootstrapAdmins(env) {
  ...
  for (const [email, password] of accounts) {
    const exists = await env.DB.prepare("SELECT email FROM yamamoto_admins WHERE email=?").bind(email).first();
    if (exists) continue;
    const salt = crypto.randomUUID();
    const hash = await pbkdf2Hex(password, salt);
    await env.DB.prepare(
      "INSERT INTO yamamoto_admins(email,password_hash,salt,created_at) VALUES (?,?,?,?)"
...
async function listProducts(env, includeDraft = false) {
  const sql = includeDraft
    ? "SELECT * FROM yamamoto_products ORDER BY CASE status WHEN 'published' THEN 0 WHEN 'draft' THEN 1 WHEN 'sold' THEN 2 ELSE 9 END, updated_at DESC"
    : "SELECT * FROM yamamoto_products WHERE status IN ('published','sold') ORDER BY CASE status WHEN 'published' THEN 0 WHEN 'sold' THEN 1 ELSE 9 END, updated_at DESC";
  const { results } = await env.DB.prepare(sql).all();
  return results.map(normalizeProduct);
}

async function getProduct(env, id, includeDraft = false) {
  const row = await env.DB.prepare("SELECT * FROM yamamoto_products WHERE id=?").bind(id).first();
  if (!row) return null;
...
async function createProduct(request, env) {
  ...
  const now = nowIso();
  if (body.featured) await env.DB.prepare("UPDATE yamamoto_products SET featured=0 WHERE featured=1").run();
  await env.DB.prepare(`
    INSERT INTO yamamoto_products
...
async function updateProduct(request, env, id) {
  ...
  const body = productPayload(await request.json().catch(() => ({})));
  if (!body.title) return json({ error: "機械タイトルを入力してください。" }, 400);
  if (body.featured) await env.DB.prepare("UPDATE yamamoto_products SET featured=0 WHERE featured=1 AND id<>?").bind(id).run();
  const now = nowIso();
  await env.DB.prepare(`
    UPDATE yamamoto_products SET title=?,category=?,description=?,video_url=?,price_mode=?,price=?,status=?,featured=?,images_json=?,specs_json=?,line_id=?,updated_at=? WHERE id=?
...
async function deleteProduct(request, env, id) {
  const admin = await verifyAdmin(request, env);
  if (!admin) return json({ error: "ログインが必要です。" }, 401);
  const result = await env.DB.prepare("DELETE FROM yamamoto_products WHERE id=?").bind(id).run();
...
async function handleApi(request, env, url) {
  ...
  if (path === "/api/logout" && request.method === "POST") {
    const token = getCookie(request, SESSION_COOKIE);
    if (token) await env.DB.prepare("DELETE FROM yamamoto_sessions WHERE token=?").bind(token).run();
