...
const DEMO_PRODUCTS = [{
 ...
 description:"在庫・仕様・状態に応じて価格をご案内する商品です。詳しい価格はLINEからメールを送ってご確認ください。",
...
function renderAdmin() {
  ...
  $("machineList").innerHTML = currentProducts.map(p => `
    <div class="machine-row ${p.status==='sold'?'sold-admin-row':''}">
      ...
      ${p.status==='published'?`<button class="btn sold-action" onclick="setSoldStatus('${esc(p.id)}','sold')">売約済みにする</button>`:''}
      ${p.status==='sold'?`<button class="btn" onclick="setSoldStatus('${esc(p.id)}','published')">販売中に戻す</button>`:''}
...
async function setSoldStatus(id,status){
  ...
  try{
    await api("/api/admin/products/"+encodeURIComponent(id),{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({...p,status})});
  }catch(_){
    const products=demoProducts().map(x=>x.id===id?{...x,status}:x);
...
function addSpec(key="", value="") {
  ...
  wrap.innerHTML=`<div class="spec-line"><input class="skey" list="suggestedKeys" value="${esc(key)}" placeholder="仕様項目名"><input class="sval" value="${esc(value)}" placeholder="値"><button class="btn remove" type="button" onclick="this.parentElement.parentElement.remove()">削除</button></div>`;
